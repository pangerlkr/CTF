...
export const PaymentBypass = () => {
  ...
  const applyCashback = () => {
    if (!cashbackApplied) {
      ...
      // Vulnerability: Cashback points increase every time, even when applying to same order
      const earnedPoints = Math.floor(subtotal * 10); // 10% back in points
  ...
  const completeOrder = () => {
    ...
    // Check if they exploited the cashback system
